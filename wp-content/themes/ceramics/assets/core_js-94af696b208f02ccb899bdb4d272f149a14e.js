  ...
  dataGroup: function(group, key, value) {
    ...
    if (key === undefined && value === undefined) {
      ...
      while (i--) {
        ...
        props[key] = String.convertValue(attr.value);
      }
      return props;
    } else if (typeOf(key) == 'string' && value === undefined) {
      return String.convertValue(this.getAttribute('data-' + group + key.hyphenate()));
...
var TouchScroll = new Class({
  ...
  Binds: ['touchStart', 'touchMove', 'touchEnd', 'touchCancel', 'orientationChanged'],
  ...
  attach: function() {
    ...
    this.orientation = (wsize.y < wsize.x) ? 'landscape' : 'portrait';
    ...
    if (this.orientationEvent) {
      window.addEvent(this.orientationEvent, this.orientationChanged);
    }
  },
  ...
  detach: function() {
    ...
    if (this.orientationEvent) {
      window.removeEvent(this.orientationEvent, this.orientationChanged);
    }
  ...
  orientationChanged: function() {
    if (this.handleOrientation) return;
  ...
  gotoPage: function(index, options) {
    this.fireEvent('startPageChanging');
    ...
    var completeHandler = function() {
      ...
      this.fireEvent('finishPageChanging');
    }.bind(this);
  ...
  touchMove: function(e) {
    ...
    this.fireEvent('touchSlideStart');
    ...
    if (!this.options.infinity) {
      ...
      */
      ...
      if ( ((this.currentPage <= 0) && (this.nextPage == -1) && (newX > 0)) ||
           ((this.currentPage >= (this.pages.length - 1)) && (this.nextPage == -1) && (newX < 0))
      ) {
        newX = this.options.bounce ? this.x + (deltaX / 3) : newX >= 0 || this.maxScrollX >= 0 ? 0 : this.maxScrollX;
      }
    }
  ...
  saveSize: function() {
    if (this.options.keepSize) {
      this.origSize = {width: this.element.style.width, height: this.element.style.height};
      var size = this.element.getSize();
      this.element.setStyles({
        width: size.x,
        height: size.y
  ...
  restoreSize: function() {
    if (this.options.keepSize) {
      this.element.setStyles(this.origSize);
      this.origSize = null;
    }
  },
  ...
  setTransitionProperty: function(el) {
    ...
    if (this.options.transition.property) property += ',' + this.options.transition.property;
  ...
  setTransition: function(el, duration) {
    ...
    if (this.options.transition.duration) duration += ',' + this.options.transition.duration;
